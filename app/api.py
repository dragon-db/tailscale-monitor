...
def create_app(config: AppConfig, storage: Storage, scheduler: MonitorScheduler) -> FastAPI:
    app = FastAPI(title="tailscale-monitor")
    ...
    async def get_nodes() -> list[dict]:
        return await asyncio.to_thread(storage.get_current_state_all_nodes)
    ...
    async def get_transitions(limit: int = Query(default=50, ge=1, le=500)) -> list[dict]:
        return await asyncio.to_thread(storage.get_recent_transitions, limit)

    @app.get("/api/stats")
    async def get_stats() -> dict:
        return await asyncio.to_thread(storage.get_stats_summary)
