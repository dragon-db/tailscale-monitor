...
- 2026-02-19: Hardened notifier delivery behavior and env secret parsing. Added whitespace/quote normalization for `.env` secrets, Discord webhook retry handling for transient/429 failures, and Discord payload `content` fallback alongside embeds for better compatibility/visibility. Impacted: `app/config.py`, `app/notifiers/discord.py`, `app/notifiers/manager.py`, `AGENTS.md`.
